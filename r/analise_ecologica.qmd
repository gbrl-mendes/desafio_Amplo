...
- **Localizar o Pandoc.** `htmlwidgets::saveWidget(selfcontained = TRUE)` (gráficos interativos, seção 10) precisa de Pandoc pra funcionar. Sem um Pandoc de sistema já no PATH (ex. instalado junto com RStudio/Quarto), usa o binário portátil baixado por `r/install_packages.R` via o pacote R `pandoc`, registrando o caminho pra esta sessão.
...
- **Construir o dendrograma como objeto ggplot2.** Usa `ggdendro::ggdendrogram()` pra converter o `hclust` num `ggplot2` normal, mesmo padrão dos outros 3 gráficos desta análise -- permite reaproveitar `ggsave` (PNG) e `plotly::ggplotly` (versão interativa, ver seção 10) sem duplicar a lógica do gráfico.
...
- **Salvar uma versão interativa (Plotly) de um gráfico ggplot2.** Envolve o mesmo objeto `ggplot2` já construído com `plotly::ggplotly()` -- não recalcula nem duplica a lógica de nenhum gráfico, só embrulha o resultado existente -- e salva como fragmento HTML autocontido (`selfcontained = TRUE`: funciona sem internet e sem depender de outros arquivos, só abrindo o HTML).
...
- **Rodar a análise inteira e exportar tabelas + gráficos.** Cada seção degrada graciosamente (pula com aviso) se faltar o dado que ela precisa -- nunca derruba as demais. Abre um dispositivo gráfico PDF próprio, redirecionado pro diretório temporário do SO, antes de qualquer gráfico: `ggplot2::ggsave()` e `plotly::ggplotly()`/`htmlwidgets::saveWidget()` (usados abaixo) podem disparar a abertura implícita do dispositivo padrão do R, que numa sessão não-interativa via `Rscript` significa um arquivo `Rplots.pdf` na pasta de trabalho (a raiz do repo, por causa do `setwd(REPO_ROOT)` no topo deste arquivo) -- sem esse dispositivo próprio já aberto, esse arquivo indesejado aparece no projeto a cada execução.
