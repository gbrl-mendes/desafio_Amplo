...
## 8. Padronização Final das Colunas de Output

Por decisão do Gabriel, o CSV exportado é reordenado/filtrado pra ficar compatível com o output "long" histórico do projeto (aba "Resultados completos" de `eDNA_Cipo--Complete_results-2025-09-17.xlsx`): colunas em comum entram na mesma ordem do arquivo original; colunas que são versões renomeadas/corrigidas de uma coluna original (`Possible target taxon` no lugar de `Possible Metazoa`; `Ponto`/`Latitude`/`Longitude`/`Habitat`/`Rios` no lugar de `Metadata 1`/`8`/`9`/`10`/`11`) ficam na posição que a coluna original ocupava; colunas 100% novas deste sistema (sem equivalente no original — vizinhos filogenéticos, contagem GBIF, Fold Change de contaminação, presença de controle) são mantidas no final do output. Colunas puramente internas de processamento (`query_taxID`, ranks "extra" do taxize como `superclass`/`infraclass`, `Relative Abundance Sample`) continuam calculadas e usadas durante o pipeline, mas não aparecem no CSV final.

- **Ordem de colunas do output "long" histórico.** Cabeçalho real da aba "Resultados completos" — usado só para reordenar/selecionar a saída final, não influencia nenhum cálculo.
...
- **Colunas renomeadas equivalentes.** Mapeia o nome da nossa coluna para o nome da coluna original que ela substitui — assim a coluna nova ocupa a posição da antiga em vez de ser tratada como novidade sem relação.
...
- **Colunas 100% novas.** Sem equivalente no output original — mantidas no final, na ordem em que são calculadas pelo pipeline.
...
- **Finalizar colunas do output.** Reordena/seleciona: colunas comuns (ou suas equivalentes renomeadas) na ordem do output histórico, seguidas das colunas 100% novas. Qualquer outra coluna (interna/de processamento) é descartada do export final.
...
- **Executar o pipeline completo.** Encadeia todos os blocos (1 a 6), finaliza as colunas do output (seção 8), imprime um resumo de diagnóstico no console e, se `--output` foi informado, exporta a tabela final via `write_csv2`.
