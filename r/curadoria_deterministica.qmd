---
title: "Curadoria Determinística desafio_Amplo"
...
Este documento descreve, em blocos, a curadoria taxonômica determinística da tabela de ASVs do projeto **desafio_Amplo**. Nenhuma etapa aqui depende de interpretação de um LLM, já que tudo aqui é reproduzível a partir de parâmetros declarados em configuração (YAML), com os defaults documentados ao longo deste documento, seção a seção.
...
- **Resolver a raiz do repo e fixar o diretório de trabalho.** Funciona via `Rscript` ou de dentro do RStudio/Positron, e fixa `setwd(REPO_ROOT)` para que caminhos relativos (`data/example/...`) resolvam de forma consistente. Com `CURADORIA_TESTING=true` (source() a partir de `r/tests/testthat`), usa um marcador de arquivo em vez de `commandArgs()`, já que nesse caso o `--file=` da linha de comando aponta pro test runner, não pra este arquivo, e o working directory já pode ter sido alterado pelo testthat antes de chegar aqui.
...
- **Defaults de configuração.** Parâmetros do pipeline com valores default para este projeto. Override via `--config`, não editando aqui.
...
- **Função de carregamento.** Une os defaults com um YAML de usuário (merge recursivo via `modifyList`; só sobrescreve as chaves que o YAML de fato traz). `colunas_alias` é mesclado à parte, por nome: é um vetor nomeado (não uma `list()`), e `modifyList` só mescla recursivamente quando os dois lados são `list()` -- sem esse tratamento, declarar qualquer alias no `--config` substituiria o default inteiro (perdendo os de Metadata 1/8/9/10/11) em vez de só complementar.
...
### Diagnósticos estruturados da execução

- **Lista acumuladora.** Cada bloco principal (2 a 6) alimenta `pipeline_diagnostics$etapas` com os mesmos números que também aparecem no console, em vez de serem strings soltas -- `main()` usa essa mesma fonte pra imprimir as mensagens e pra gravar `<saida>_diagnostics.json` ao lado do CSV de saída.
...
- **Formatar contagens por categoria.** Uma linha de texto tipo `"Species: 62, Genus: 2, sem dado: 16"` a partir de uma coluna -- usado nos diagnósticos de cada bloco em vez de imprimir a tabela crua do R.
...
### Nomes de coluna flexiveis

- **Aplicar o mapa de alias de colunas.** Renomeia QUALQUER coluna bruta (obrigatoria, dos hits de BLAST, ou de metadado semantico) para o nome canonico que o resto do pipeline usa (`config$colunas_alias`), e converte `Latitude`/`Longitude` de inteiro para grau decimal (÷1e6). Roda antes de `check_required_columns` -- para que um arquivo com nomes diferentes do canonico ainda passe pela checagem de colunas obrigatorias depois de renomeado.
...
- **Header da ASV.** Formato `">ASV_{posição}-{tamanho}bp"`, posição = rank por tamanho decrescente entre sequências únicas (empate por abundância total).
...
- **Setup + parsing.** Lê o CSV bruto, aplica o mapa de alias de colunas (`config$colunas_alias`) — para que nomes de coluna diferentes do canônico já sejam traduzidos antes de qualquer checagem —, confere colunas obrigatórias, corrige metadados geográficos e calcula as 3 colunas derivadas localmente.
...
- **Padrões de resultado ruim.** Headers "não-informativos" do BLAST (ex. "Uncultured bacterium") que não contam como identificação válida.
...
- **Consultar taxonomia no NCBI.** Consulta o NCBI (`taxize::classification`) para os taxids únicos de `query_taxID` (do hit BLAST já selecionado) e devolve uma linha por taxid, uma coluna por nível taxonômico. O nível mais alto retornado pelo taxize se chama `"superkingdom"`.
...
- **Preencher ranks faltantes.** `Superkingdom` via regras a partir de `Kingdom`/`Phylum` quando ausente; ranks intermediários viram `"{rank} of {rank superior}"` em vez de `NA`.
...
- **Unir taxonomia e finalizar.** Une a taxonomia completa de volta pela `query_taxID` e corrige `BLAST ID` quando o hit BLAST veio abreviado (gênero em uma letra + ponto, ex. `"P.rivularis algo"`), substituindo pelo gênero completo resolvido no NCBI.
...
- **Valores de `Type` que identificam um controle.** Variantes de nomenclatura observadas no contrato de schema.
...
- **Função de fold change.** Equivalente a `gtools::foldchange(num, denom)` (FC = num/denom se num \>= denom, senão -denom/num).
...
- **Máximo seguro.** `max(x, na.rm = TRUE)` sem o aviso "no non-missing arguments", devolvendo `NA_real_` (não `-Inf`) quando o grupo é todo `NA`.
...
- **Detectar faixa de amplicon automaticamente quando não declarada.** Pra cada primer presente nos dados sem entrada em `amplicon_por_primer`, estima a faixa esperada a partir da própria distribuição de `ASV Size (pb)` daquele primer: quartis 25%/75% ± 1.5×IQR (mesmo critério estatístico padrão de detecção de outliers, ex. boxplot) — sequências bem mais curtas ou mais longas que o grosso da distribuição (prováveis primer-dimer/artefato) ficam de fora. Com menos de 4 ASVs desse primer, não há dado suficiente pra estimar; a faixa fica indefinida (mesmo efeito de um primer não declarado, ver abaixo). Chamada uma única vez em `main()`, logo após o setup, substituindo `config$amplicon_por_primer` pelo resultado -- assim tanto a curadoria final quanto o piso de tamanho da árvore filogenética (bloco 5) enxergam a mesma faixa resolvida, em vez de cada bloco resolver por conta própria.
...
- **Marcar faixa de amplicon.** Compara `ASV Size (pb)` com a faixa esperada de amplicon do primer (declarada ou detectada automaticamente, ver acima). Primer sem faixa nenhuma → `NA` (não assumimos "fora da faixa" para um primer sem dado suficiente).
...
- **Registro interno de táxons-alvo.** Associa cada nome de grupo (usado em `config$taxons_alvo$grupos`) a um reino catch-all opcional e às listas de filo/classe que o compõem. Cobre os grupos tipicamente monitorados num projeto de eDNA multi-primer (peixes via Metazoa, bentos, zooplâncton, fitoplâncton, perifíton, plantas via Plantae).
...
A área consultada no GBIF é calculada a partir dos próprios pontos amostrados (min/max de `Latitude`/`Longitude` nos dados, expandido por `config$checagem_regional$buffer_graus` em cada lado), a menos que `config$checagem_regional$area_bbox` seja declarado explicitamente, que aí é usado como está. O script busca só o fato (quantos registros do GBIF existem na área); plausibilidade é julgamento de uma etapa posterior.

- **Calcular a área a partir dos pontos amostrados.** `area_bbox` explícito tem prioridade; senão, usa o min/max de `Latitude`/`Longitude` presentes nos dados, expandido por `buffer_graus`; sem `area_bbox` e sem `Latitude`/`Longitude`, retorna `NULL`.
...
- **Construir o polígono WKT.** Monta o polígono WKT (ordem "longitude latitude", como o GBIF espera) a partir do bounding box calculado.
...
- **Contar registros regionais no GBIF.** Conta registros do GBIF pra um nome científico dentro da área declarada, via `rgbif::occ_count()` (wrapper leve de `occ_search(limit=0)$meta$count`, que aceita `geometry=` na versão instalada). `NA` (não `0`) se a consulta falhar, não confundir "sem registro" com "não consultou" (rede indisponível, nome não resolvido no GBIF etc.). Uma pausa de 0.3s depois de cada chamada evita disparar o limite de taxa da API pública do GBIF, mesmo padrão já usado na consulta de taxonomia do NCBI (`retrieve_taxonomy`); se mesmo assim vier "Too many requests", tenta de novo até `max_retries` vezes com espera exponencial (mesmo padrão do retry da Groq em `llm_curation.py::call_groq`) antes de desistir. Erros de outro tipo (nome não resolvido, rede indisponível) não são transitórios e vão direto pra `NA_integer_`.
...
O CSV exportado é reordenado e filtrado para compatibilidade com o formato "long" histórico do projeto: colunas em comum entram na mesma ordem do arquivo original; colunas renomeadas ou corrigidas (`Possible target taxon` no lugar de `Possible Metazoa`; `Ponto`/`Latitude`/`Longitude`/`Habitat`/`Rios` no lugar de `Metadata 1`/`8`/`9`/`10`/`11`) ficam na posição da coluna que substituem; colunas novas deste sistema (vizinhos filogenéticos, contagem GBIF, Fold Change de contaminação, presença de controle) ficam no final. Colunas internas de processamento (`query_taxID`, ranks "extra" do taxize, `Relative Abundance Sample`) são usadas durante o pipeline, mas não aparecem no CSV final.

- **Ordem de colunas do output "long" histórico.** Herdada do formato histórico do projeto; usada só para reordenar a saída final, não influencia nenhum cálculo.
...
- **Colunas renomeadas equivalentes.** Mapeia o nome da coluna nova para o nome da coluna original que ela substitui.
...
- **Executar o pipeline completo.** Encadeia todos os blocos (1 a 6), finaliza as colunas do output (seção 8), imprime um resumo de diagnóstico no console (a partir de `pipeline_diagnostics`, alimentada por cada bloco) e, se `--output` foi informado, exporta a tabela final via `write_csv2` e os diagnósticos estruturados em `<saida sem .csv>_diagnostics.json`, ao lado do CSV.
