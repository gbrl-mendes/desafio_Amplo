...
- **Função de carregamento.** Une os defaults com um YAML de usuário (merge recursivo via `modifyList`; só sobrescreve as chaves que o YAML de fato traz).
...
- **Registro interno de táxons-alvo.** Associa cada nome de grupo (usado em `config$taxons_alvo$grupos`) a um reino catch-all opcional e às listas de filo/classe que o compõem. Cobre os grupos tipicamente monitorados num projeto de eDNA multi-primer (peixes via Metazoa, bentos, zooplâncton, fitoplâncton, perifíton).
